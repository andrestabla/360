...
export default function UsersPage() {
    ...
    const handleResendCredentials = async () => {
        ...
        setLoading(true);
        const result = await sendWelcomeEmailAction(editingId, passwordToSend);
        if (result.success) {
            addNotification({
                title: 'Éxito',
                message: autoGenerated
                    ? `Credenciales enviadas. Se generó: ${passwordToSend}`
    ...
    useEffect(() => {
        if (activeTab === 'activity' && editingId) {
            loadActivityLogs(editingId);
        }
    }, [activeTab, editingId]);
                                    ...
                                    <div className="space-y-2">
                                        {isLoadingLogs ? (
                                            ...
                                            activityLogs.map((log: any) => (
                                                <div key={log.id} className="flex items-center justify-between py-3 border-b border-slate-100">
                                                    ...
                                                    <div className="text-xs text-slate-400 w-24 text-right truncate" title={log.ipAddress}>
                                                        {log.ipAddress || '-'}
                                                    </div>
                                                </div>
                                            ))
                                        ...
                                        {!isLoadingLogs && activityLogs.length > 0 && (
                                            <div className="text-center py-4 text-slate-400 text-xs mt-4 border-t border-slate-50">
                                                Mostrando últimos {activityLogs.length} eventos
                                            </div>
                                        )}
                                    </div>
                                </div>
