...
export default function UnitsPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const dataToSave = {
            ...formData,
            // ID will be generated by action if not editingId
        };

        if (editingId) {
            await updateUnit(editingId, dataToSave);
        } else {
            await createUnit(dataToSave);
        }
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fadeIn">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 mb-1">Código (Obligatorio)</label>
                                    <input
                                        required
                                        className="w-full px-3 py-2 border rounded-lg focus:ring-2 ring-blue-500/20 outline-none transition-all"
                                        value={formData.code || ''}
